...
function Payment() {
  ...
  return (
    ...
    <CommonContainer>
      <RegistrationHeader
        title={"Pagamentos"}
        subtitle={"Selecione seu método de pagamento"}/>

      <div className="buttons mb-5 buttonsPayment">
        <button className={`button ${paymentMethod === 'creditCard' ? 'is-primary' : ''}`}
          onClick={() => handlePaymentMethodChange('creditCard')}>Cartão de Crédito
        </button>
        <button className={`button ${paymentMethod === 'debitCard' ? 'is-primary' : ''}`}
          onClick={() => handlePaymentMethodChange('debitCard')}>Cartão de Débito
        </button>
        <button className={`button ${paymentMethod === 'pix' ? 'is-primary' : ''}`}
          onClick={() => handlePaymentMethodChange('pix')}>Pix
        </button>
      </div>
        ...
        <div>
          ...
          <section className='buttonsPayment'>
            <button className="button" onClick={() => {
              ...
              setPixKey('37c2a65d-e018-4f17-8a35-593b1b25816d');
            }}>Chave Copia e Cola
            </button>
